...
class Pasajeros extends Component {
    ...
    render(){
        const {
            onChange,
            state:{show,adulto,niño,infante,
        }}=this;
        ...
        let disabledMenosInfantes=true;
        let disabledMasInfantes=false;
        ...
        if (infante===2) {disabledMasInfantes=true};
        if (infante>=1) {disabledMenosInfantes=false};
        ...
        const masClickInfante =()=>this.setState({infante:infante+1});
        const menosClickInfante =()=>this.setState({infante:infante-1});
        
        let string=adulto+niño+infante+' pasajeros';

        if(adulto===0 && niño===0 && infante===0 ){
        ...
        return (
            <>
                ...
                <div className='PasajerosOpcion'>
                    <h1>{infante} Infantes</h1>
                    <button onClick={menosClickInfante} disabled={disabledMenosInfantes}>-</button>
                    <button onClick={masClickInfante} disabled={disabledMasInfantes}>+</button>
                </div>
                </Modal>
            </>
        )
